import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:expenz/models/income_category_model.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';

class IncomeServices {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final CollectionReference _incomeCollection = FirebaseFirestore.instance
      .collection('incomes');

  Future<String?> _currentUserId() async {
    return FirebaseAuth.instance.currentUser?.uid;
  }

  Future<void> saveIncomes(Income income, BuildContext context) async {
    try {
      final userId = await _currentUserId();
      if (userId == null) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please sign in to save incomes.')),
          );
        }
        return;
      }

      await _firestore
          .collection('users')
          .doc(userId)
          .collection('incomes')
          .doc(income.category.toString())
          .set(income.toJson());

      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Incomes added successfully...'),
            duration: Duration(seconds: 2),
          ),
        );
      }
    } catch (error) {
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Error...')));
      }
    }
  }

  Future<List<Income>> loadIncome() async {
    final userId = await _currentUserId();
    if (userId == null) return [];

    final snapshot = await _firestore
        .collection('users')
        .doc(userId)
        .collection('incomes')
        .get();

    return snapshot.docs.map((doc) => Income.fromJSON(doc.data())).toList();
  }

  Future<void> removeIncome(String id, BuildContext context) async {
    try {
      final userId = await _currentUserId();
      if (userId == null) return;

      await _firestore
          .collection('users')
          .doc(userId)
          .collection('incomes')
          .doc(id)
          .delete();

      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Incomes deleted successfully'),
            duration: Duration(seconds: 2),
          ),
        );
      }
    } catch (error) {
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Error...')));
      }
    }
  }

  Future<void> deleAllIncomes(BuildContext contex) async {
    try {
      final userId = await _currentUserId();
      if (userId == null) return;

      final snapshot = await _firestore
          .collection('users')
          .doc(userId)
          .collection('incomes')
          .get();

      for (final doc in snapshot.docs) {
        await doc.reference.delete();
      }

      if (contex.mounted) {
        ScaffoldMessenger.of(
          contex,
        ).showSnackBar(const SnackBar(content: Text('All incomes deleted')));
      }
    } catch (err) {
      if (contex.mounted) {
        ScaffoldMessenger.of(
          contex,
        ).showSnackBar(const SnackBar(content: Text('Error')));
      }
    }
  }
}
